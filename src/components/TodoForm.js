const TodoForm = ({handleTodoAdd, handleTitleChange, title, handleDescriptionChange, description, isEditMode, handleTaskUpdate, titleField
    ...
    return(
        <form id='TodoForm' className="flex flex-col justify-items-stretch" onSubmit={!isEditMode ? handleTodoAdd : handleTaskUpdate}>
            ...
            <textarea onChange={handleDescriptionChange} value={description} name="description" id="description" cols="30" rows="10" placeholder="Description" className="mb-3 drop-shadow p-2 text-sm"></textarea>
            <button type="submit" className="bg-secondary py-1 text-white text-l">Add</button>
            {isEditMode ? <button onClick={handleCancelEdit} className="bg-tertiary py-1 text-white text-l mt-3">Cancel</button> : null}
        </form>
    );
};
